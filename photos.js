...
function openPhotoModal(index) {
  ...
  photoModal.innerHTML = `
    <div class="modal-content" style="max-width: 90vw; max-height: 90vh; padding: 0; border-radius: 8px; position: relative; background: var(--card-bg); display: flex; flex-direction: column; overflow: hidden;">
      <div class="modal-header">
        <h3>Фото-отчет по объекту</h3>
        <span class="modal-close" id="closePhotoModal">&times;</span>
      </div>
      <div style="padding: 12px 16px; background: #f2f2f2; font-weight: 500; font-size: 14px; color: #333;">
        ${title}
      </div>
      <div style="padding: 20px; text-align: center; background: white;">
        <img src="images/${img}" alt="Фото" style="max-width: 100%; max-height: 70vh; object-fit: contain; border-radius: 6px;" />
      </div>
    </div>
  `;
